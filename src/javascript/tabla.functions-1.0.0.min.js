...
function filterCommand(command) {
  ...
  for (i = 0; i < res.length; i++) {
    if (res[i] === "filter") {
      ...
      attribute = checkAttributeIsDate(attribute);
      filter = true;
    } else if (filter && res[i - 1] === "to") {
      while (i < res.length) {
        value += correctAttributeWordToStandard(res[i++]) + " ";
        console.log(value);
      }
      value = value.trim();
    }
...
function checkAttributeIsDate(attribute) {
  console.log(attribute.toLowerCase());
  if (attribute.toLowerCase().indexOf("year") !== -1) {
    console.log("FOUND");
    return "YEAR(Date)";
  }
  return attribute;
